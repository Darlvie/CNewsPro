...
- (NSInteger)addPlaceList:(NSMutableArray *)placeList {
    ...
    NSString *sql = @"INSERT INTO Place (code,name,language)VALUES (?,?,?)";
    ...
    {
        Place *place=[placeList objectAtIndex:i];
        //绑定参数
        sqlite3_bind_text(statement, 1, [place.code UTF8String], -1, NULL);
        sqlite3_bind_text(statement, 2, [place.name UTF8String], -1, NULL);
        sqlite3_bind_text(statement, 3, [place.language UTF8String], -1, NULL);
...
- (BOOL)deleteAll {
    ...
    NSString *sql = @"DELETE FROM Place";
    sqlite3_stmt *statement = nil;
    int success = [self prepareSQL:sql SQLStatement:&statement];
    if (success != SQLITE_OK) {
